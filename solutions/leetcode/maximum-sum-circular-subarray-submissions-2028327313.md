# maximum sum circular subarray/submissions/2028327313
...
public:
    int maxSubarraySumCircular(vector<int>& nums) {
        ...
        for(int i=1;i<nums.size();i++){
            ...
            best_max=max(c1,c2);//-2
            best_min=min(c1,c3);//-5
            ans_min=min(ans_min,best_min);//-5
            ans_max=max(ans_max,best_max);//-2
          }
          if(ans_min==total)
          return ans_max;
